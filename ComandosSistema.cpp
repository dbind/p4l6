...
void ComandosSistema::loadTestData()
{
	...
	cSesion->iniciarSesionInterno(uLogueado); // Restaurar sesi�n previa
	...
	// Representaciones (Categor�a, C�digo, Descripci�n)
	Representacion R1 = cDiagnostico->altaRepresentacion(X1, "01", "Asma");
	Representacion R2 = cDiagnostico->altaRepresentacion(X1, "02", "Congesti�n");
	Representacion R3 = cDiagnostico->altaRepresentacion(X2, "01", "Nauseas");
	

	// Diagn�sticos (Consulta, Representaci�n, Descripci�n)
	...
	// Tratamientos (Diagn�stico, F�rmaco, Descripci�n)
...
void ComandosSistema::listarReservas()
{
	...
	for (; it != l.end(); ++it)
	{
		...
		cout << "M�dico: " << (*it)->medico()->getNombre() << " " << (*it)->medico()->getApellido() << endl
...
void ComandosSistema::listarConsultas()
{
	...
	for (; it != l.end(); ++it)
	{
		...
		else
		{
			...
			cout << "Descripci�n: " << urgente->descripcion() << endl << endl;
		}
	}	
}
